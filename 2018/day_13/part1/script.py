...
def reset_char(grid, x, y):
    ...
    acessible = [0, 0, 0, 0]
    if x > 0 and grid[y][x - 1] in movable_chars:
        acessible[0] = 1
    if x < (len(grid[y]) - 1) and grid[y][x + 1] in movable_chars:
        acessible[1] = 1
    if y > 0 and grid[y - 1][x] in movable_chars:
        acessible[2] = 1
    if y < (len(grid[y]) - 1) and grid[y + 1][x] in movable_chars:
        acessible[3] = 1
...
def move_cart(grid, cart):
    ...
    if len(grid[cart.y][cart.x]) == 2:
        raise Exception('CRASH! {},{}'.format(cart.x, cart.y))
